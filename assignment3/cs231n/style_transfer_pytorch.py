...
def gram_matrix(features, normalize=True):
    ...
    a = features.view(N, C, H * W)

    gram = torch.matmul(a, torch.transpose(a, 1, 2))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, layer in enumerate(style_layers):
      a = (gram_matrix(feats[layer]) - style_targets[i])**2
      loss += style_weights[i] * torch.sum(a) 
...
def tv_loss(img, tv_weight):
    ...
    vertical = (img[:, :, :-1, :] - img[:, :, 1:, :])**2
    horizontal = (img[:, :, :, :-1] - img[:, :, :, 1:])**2
    loss = tv_weight * (torch.sum(vertical) + torch.sum(horizontal))
    return loss
